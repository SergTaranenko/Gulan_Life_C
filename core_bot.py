...
def rank_status_text(data: dict) -> str:
    ...
    if idx == 11:
        ...
        lines.append(f"⚒️ Всего дел: {data['total_deeds']}/174")
    else:
...
async def cmd_tried(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = load_data()
    # Сдвигаем last_deed_time вперёд на 4 часа = +4ч сытости
    ...
    save_data(data)
    phrases = [
        "Попытка — не провал. Огонь сохранён. +4ч",
        "Ты старался. Это засчитано в кости. +4ч",
        "Иногда попытка — это всё, что есть. Принято. +4ч",
        "Работа была. Результата нет. Продолжай. +4ч"
    ]
...
async def cmd_penalty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.get("last_deed_time"):
        ...
        if last.tzinfo is None:
            last = TIMEZONE.localize(last)
        data["last_deed_time"] = (last - timedelta(hours=1)).isoformat()
    data["hunger_notified"] = False
    save_data(data)
    rank = get_rank_data(data["rank_index"])
    deeds = data["rank_deeds"]
...
async def cmd_penalty20(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.get("last_deed_time"):
        ...
        if last.tzinfo is None:
            last = TIMEZONE.localize(last)
        data["last_deed_time"] = (last - timedelta(hours=20)).isoformat()
    data["hunger_notified"] = False
    save_data(data)
    rank = get_rank_data(data["rank_index"])
    deeds = data["rank_deeds"]
