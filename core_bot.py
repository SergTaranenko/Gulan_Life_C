# -*- coding: utf-8 -*-
"""
Бот «12 рангов» v3 — Путь через эпохи.
Ядро — engine. Контент рангов в ranks/rank_01.py … rank_12.py.
"""
...
RANKS_DIR     = Path(__file__).parent / "ranks"
...
# ── Inline Клавиатура ───────────────────────────────────────────────────────
def main_keyboard():
    """Inline-кнопки под каждым сообщением — работают в Telegram Web"""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⚒️ Сделано", callback_data="done"),
         InlineKeyboardButton("🤔 Пытался", callback_data="tried")],
        [InlineKeyboardButton("❌ Неудача", callback_data="penalty"),
         InlineKeyboardButton("📊 Статус", callback_data="status")],
        [InlineKeyboardButton("🎖 Ранг", callback_data="rank"),
         InlineKeyboardButton("🗺 Путь", callback_data="path")],
        [InlineKeyboardButton("🧊 Шаман", callback_data="shaman"),
         InlineKeyboardButton("🎁 Награда", callback_data="reward")],
...
RANK_SCHEDULE = [
    ...
    {"rank_idx": 10, "start": "2026-11-09", "active_from": "2026-11-24"},
    {"rank_idx": 11, "start": "2026-12-12", "active_from": "2026-12-14"},
]
VICTORY_DATE = date(2026, 12, 31)
BOT_END_DATE = date(2027, 1, 2)
...
class GigaChatAPI:
    ...
    async def generate_image(self, prompt: str):
        ...
        if not token:
            logger.warning("[IMG] No GigaChat token — images disabled")
            return None
        ssl_ctx = ssl.create_default_context()
        ssl_ctx.check_hostname = False
        ssl_ctx.verify_mode = ssl.CERT_NONE
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as s:
                logger.info(f"[IMG] Sending prompt: {prompt[:80]}...")
                async with s.post(
                    ...
                    json={
                        ...
                        "functions": [
                            {
                                "name": "generate_image",
                                "description": "Generate photorealistic image from text description",
                                "parameters": {
                                    "type": "object",
                                    "properties": {}
                                }
                    ...
                    if resp.status != 200:
                        logger.error(f"[IMG] HTTP {resp.status}: {body[:300]}")
                        return None
                    d = json.loads(body)
                    ...
                    if '<img src="' in content:
                        ...
                        logger.info(f"[IMG] Got file_id: {file_id}")
                        async with s.get(
                            ...
                            if img_resp.status == 200:
                                ...
                                logger.info(f"[IMG] Downloaded: {len(img_bytes)} bytes")
                                return img_bytes
                            else:
                                logger.error(f"[IMG] Download HTTP {img_resp.status}")
                    else:
                        logger.warning(f"[IMG] No <img> in response. Content: {content[:200]}")
        except Exception as e:
...
async def cmd_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Главное меню — список всех команд"""
    msg = (
        "📋 МЕНЮ КОМАНД\n\n"
        ...
        "🧊 /shaman — Ледник Шамана\n"
        "🔮 /wisdom — Мудрость эпохи\n\n"
        "Жми кнопки ниже 👇"
    )
...
async def cmd_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Расписание всех рангов"""
    lines = ["📜 РАСПИСАНИЕ 12 РАНГОВ\n"]
    current_idx = get_current_schedule_idx()

    lines.append("═══ НАТУФИЙСКАЯ КУЛЬТУРА ═══")
    for i in range(6):
        r = get_rank_data(i)
        sched = RANK_SCHEDULE[i]
        start = parse_date(sched["start"])
        active = parse_date(sched["active_from"])
        prefix = "👉" if i == current_idx else "✅" if i < current_idx else "⬜"
        date_str = f"{start.strftime('%d.%m')}→{active.strftime('%d.%m')}"
        lines.append(f"{prefix} {i+1}. «{r['name']}» [{date_str}] — {r['deeds_needed']} дел")
    ...
    total_natuf = sum(get_rank_data(i)["deeds_needed"] for i in range(6))
    total_neo = sum(get_rank_data(i)["deeds_needed"] for i in range(6, 12))
    lines.append(f"\n📊 Всего: {total_natuf}+{total_neo} = {total_natuf+total_neo} дел")
    lines.append(f"🏁 Финал: 31.12.2026")
...
async def cmd_wisdom(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    era_desc = (
        ...
        "Люди живут в полуподземных домах, собирают дикую пшеницу, охотятся на газелей."
        if rank["era"] == "natuf" else
        "Ранний неолит Анатолии, ~9500-8000 до н.э. "
        "Люди строят каменные дома, сеют зерно, возводят Гёбекли-Тепе."
    )

    prompt = (
        f"Ты — {rank['name']} в {era_desc}\n\n"
        f"Сегодняшняя заповедь: «{cmd['short']}» — {cmd['full']}\n\n"
        f"Напиши короткую сцену из жизни этого человека (1-3 абзаца). "
        f"Он сталкивается с ситуацией, связанной с этой заповедью. "
        f"Живые детали: запахи, звуки, жесты, ощущения. "
        f"Стиль: земной, телесный, без пафоса. Конкретика, а не общие фразы.\n\n"
        f"Например, если ранг 'Падальщик у края' и заповедь 'Принимай крученые мячи', "
        f"то сцена про риск, шанс, попытку — даже отпугнуть гиен от туши опасно, "
        f"но принести кости и шкуру — это шанс быть замеченным племенем."
    )

    await update.message.reply_text("🔮 Шаман читает кости...", reply_markup=main_keyboard())

    result = await gigachat.generate_text(prompt, temperature=0.9)
    if result:
        await update.message.reply_text(
            f"🔮 МУДРОСТЬ ЭПОХИ\n\n"
            f"🎖 Ранг: «{rank['name']}»\n"
            f"📜 Заповедь {cmd['id']}: «{cmd['short']}»\n\n"
            f"{result}",
            reply_markup=main_keyboard()
        )
    else:
        # Fallback если GigaChat не отвечает
        era = "Натуфа" if rank["era"] == "natuf" else "Анатолия"
        await update.message.reply_text(
            f"🔮 МУДРОСТЬ ЭПОХИ\n\n"
            f"🎖 Ранг: «{rank['name']}» ({era})\n"
            f"📜 Заповедь {cmd['id']}: «{cmd['short']}»\n\n"
            f"{rank['full']}\n\n"
            f"💡 {cmd['full']}",
            reply_markup=main_keyboard()
...
async def cmd_shaman(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(cipher) <= 4000:
        await update.message.reply_text(cipher, reply_markup=main_keyboard())
    else:
    ...
    if product_img_prompt:
        ...
        if img:
            ...
            await update.message.reply_photo(
                photo=BytesIO(img), caption=caption,
                reply_markup=main_keyboard()
            )
        else:
            # Fallback: если GigaChat недоступен, показать текстовое описание
            await update.message.reply_text(
                f"🎨 {product_name}\n📝 {product_img_prompt[:200]}...",
                reply_markup=main_keyboard()
            )
...
async def cmd_tried(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(random.choice(phrases), reply_markup=main_keyboard())
...
async def cmd_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    penalty = data.get("penalty_pool", 0)
    carry = data.get("carry_deeds", 0)
    rewards = data.get("rewards_earned", 0)

    lines = ["📊 БАЛАНС ДЕЛ\n"]
    lines.append(f"🎖 Ранг: «{rank['name']}»")
    lines.append(f"⚒️ Прогресс: {deeds}/{needed}")
    lines.append(f"📈 Всего дел: {data['total_deeds']}/174")
    if carry > 0:
        lines.append(f"📦 Кэрриовер: {carry} (уменьшает следующий ранг)")
    if excess > 0:
        lines.append(f"⚡ Сверхдел: {excess} — можно потратить на /reward")
    if penalty > 0:
        lines.append(f"🔴 Долг: {penalty} дел — закрыть через /paydebt")
    if rewards > 0:
        lines.append(f"🎁 Супернаград получено: {rewards}")

    lines.append("\nКоманды:")
    lines.append("/reward — супернаграда (1 сверхдело)")
    if penalty > 0:
        lines.append("/paydebt — погасить долг")
    if excess > 0 and idx < 11:
        lines.append("/carry — перенести в следующий ранг")

    await update.message.reply_text("\n".join(lines), reply_markup=main_keyboard())


async def cmd_paydebt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Погасить долг из избытка"""
    data = load_data()
    penalty = data.get("penalty_pool", 0)
    excess = data.get("excess_pool", 0)

    if penalty <= 0:
        await update.message.reply_text("✅ Долгов нет.", reply_markup=main_keyboard())
        return
    if excess <= 0:
        await update.message.reply_text(
            f"❌ Нет сверхдел для погашения. Сделай больше нормы.",
            reply_markup=main_keyboard()
        )
        return

    # Погашаем 1 долг за 1 сверхдело
    amount = min(penalty, excess)
    data["penalty_pool"] = penalty - amount
    data["excess_pool"] = excess - amount
    save_data(data)

    await update.message.reply_text(
        f"✅ Долг погашен: {amount}\n"
        f"🔴 Осталось долга: {data['penalty_pool']}\n"
...
async def cmd_carry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Перенести сверхдело в следующий ранг"""
    data = load_data()
    idx = data["rank_index"]
    excess = data.get("excess_pool", 0)

    if idx >= 11:
        await update.message.reply_text("👑 Ты на вершине. Некуда переносить.", reply_markup=main_keyboard())
        return
    if excess <= 0:
        await update.message.reply_text(
            "❌ Нет сверхдел. Сделай больше нормы.",
            reply_markup=main_keyboard()
        )
        return

    data["excess_pool"] = excess - 1
    data["carry_deeds"] = data.get("carry_deeds", 0) + 1
    save_data(data)

    next_rank = get_rank_data(idx + 1)
...
async def cmd_collection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Музей артефактов — коллекция выполненных дел"""
    data = load_data()
    total = data.get("total_deeds", 0)
    idx = data["rank_index"]
    rank = get_rank_data(idx)
    deeds = data["rank_deeds"]
    needed = effective_deeds_needed(data)

    lines = ["🏛️ МУЗЕЙ АРТЕФАКТОВ\n"]
    lines.append(f"⚒️ Всего дел за путь: {total}/174")
    lines.append(f"🎖 Текущий ранг: «{rank['name']}» — {deeds}/{needed}")

    # Собираем продукты из всех пройденных рангов + текущего
    all_products = []
    for r_idx in range(min(idx + 1, 12)):
        r = get_rank_data(r_idx)
        for pname, _ in r.get("products", []):
            all_products.append(f"{pname} (ранг {r_idx+1})")
        for pname, _ in r.get("rare_products", []):
            all_products.append(f"🌟 {pname} (ранг {r_idx+1})")

    if all_products:
        lines.append(f"\n📦 Артефактов в коллекции: {len(all_products)}")
        lines.append("\n".join(all_products[-10:]))  # последние 10
        if len(all_products) > 10:
            lines.append(f"\n...и ещё {len(all_products) - 10} артефактов")
    else:
        lines.append("\nМузей пуст. Начни собирать — /done")

    milestones = data.get("milestones_shown", [])
    if milestones:
        lines.append(f"\n🏆 Вехи достигнуты: {len(milestones)}")
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    data = load_data()
    await update.message.reply_text(rank_status_text(data), reply_markup=main_keyboard())
...
async def cmd_rank(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start = parse_date(sched["start"])
    active = parse_date(sched["active_from"])
    date_info = f"\n📅 {start.strftime('%d.%m')} — {active.strftime('%d.%m.%Y')}"

    await update.message.reply_text(
        f"📜 {era}  |  {era_pos}{date_info}\n"
        f"🎖 {rank['name']}\n\n"
        f"{rank['full']}\n\n"
        f"⚒️ {bar}{carry_info}",
        reply_markup=main_keyboard()
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "⚒️ Сделано":
        ...
        await cmd_path(update, context)
        return
    elif text == "🧊 Шаман":
        await cmd_shaman(update, context)
        return
    elif text == "🎁 Награда":
        await cmd_reward(update, context)
        return
    elif text == "📜 Расписание":
        await cmd_schedule(update, context)
        return
    elif text == "🔮 Мудрость":
        await cmd_wisdom(update, context)
        return
    elif text == "📊 Баланс":
        await cmd_balance(update, context)
        return
    elif text == "🏛 Коллекция":
        await cmd_collection(update, context)
        return
    elif text == "📋 Меню":
    ...
    # Утренний вопрос про 4 дела
    if data.get("waiting_for_plans"):
        if any(w in text_lower for w in ["есть", "да", "yes", "готово"]):
            data["waiting_for_plans"] = False
            data["superhero_morning_flag"] = True
            save_data(data)
            rank = get_rank_data(data["rank_index"])
            await update.message.reply_text(
                f"✅ Отлично, {rank['name']}! План есть — племя будет сыто.",
                reply_markup=main_keyboard()
            )
            ...
            if img:
                ...
                await update.message.reply_text("🌅 Рассвет. Начинай.", reply_markup=main_keyboard())
        elif any(w in text_lower for w in ["нет", "no", "не", "нету"]):
            data["waiting_for_plans"] = False
            save_data(data)
            await update.message.reply_text(
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Создаём фейковое update.message для совместимости с cmd_*
    class FakeMessage:
        def __init__(self, orig_msg):
            self._orig = orig_msg
        async def reply_text(self, text, **kwargs):
            return await self._orig.reply_text(text, **kwargs)
        async def reply_photo(self, **kwargs):
            return await self._orig.reply_photo(**kwargs)
    ...
    if data == "done":
        ...
        await cmd_rank(fake_update, context)
    elif data == "path":
        await cmd_path(fake_update, context)
    elif data == "shaman":
        await cmd_shaman(fake_update, context)
    elif data == "reward":
        await cmd_reward(fake_update, context)
    elif data == "schedule":
        await cmd_schedule(fake_update, context)
    elif data == "wisdom":
        await cmd_wisdom(fake_update, context)
    elif data == "balance":
        await cmd_balance(fake_update, context)
    elif data == "collection":
        await cmd_collection(fake_update, context)
    elif data == "menu":
