...
def rank_status_text(data: dict) -> str:
    ...
    if idx == 11:
        ...
        if carry > 0:
            lines.append(f"⚡ Кэрриовер: -{carry} дел к следующему рангу")
        lines.append(f"📊 Всего за путь: {data['total_deeds']}/174")
        lines.append(f"🎯 До следующего ранга: {max(0, needed - deeds)}")
...
async def check_date_transitions(bot, user_id: int, data: dict):
    ...
    for sched in RANK_SCHEDULE[1:]:   # пропускаем ранг 0 (старт)
        ...
        if today >= parse_date(start_str) and start_str not in shown:
            ...
            # Считаем кэрриовер
            if new_idx > old_idx:
                old_rank = get_rank_data(old_idx)
                needed = effective_deeds_needed(data)
                excess = max(0, data["rank_deeds"] - needed)
                data["carry_deeds"] = excess
                ...
                if today < active_from:
                    ...
                    if excess > 0:
                        new_needed = max(1, new_rank["deeds_needed"] - excess)
                        msg += f"\n⚡ Запас дел из прошлого ранга: {excess}. Следующий ранг: {new_needed} дел."
